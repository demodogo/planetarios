...
function ModeloVariablePopover({
    ...
    return (
        ...
        >
            {onClose ? (
                ...
                >
                    <span className="relative block h-4 w-4">
                        <span className={`absolute left-1/2 top-1/2 block h-[1.5px] w-4 -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-full ${textColor === 'white' ? 'bg-white' : 'bg-black/75'}`} />
                        <span className={`absolute left-1/2 top-1/2 block h-[1.5px] w-4 -translate-x-1/2 -translate-y-1/2 -rotate-45 rounded-full ${textColor === 'white' ? 'bg-white' : 'bg-black/75'}`} />
                    </span>
                </button>
            ) : null}

            <div className={`relative ${mobileFullscreen ? 'h-[10.5rem]' : 'h-[8.5rem]'} md:h-auto md:w-[13.25rem]`}>
                <div className={`absolute left-1/2 -translate-x-1/2 -translate-y-1/2 md:-left-[6rem] md:-top-[8rem] md:translate-x-0 md:translate-y-0 ${mobileFullscreen ? 'top-[34%] h-[13rem] w-[13rem]' : 'top-[52%] h-[11rem] w-[11rem]'} md:h-[25.875rem] md:w-[25.875rem]`}>
            ...
            <div className={`${mobileFullscreen ? 'px-5 pb-7 pt-1 text-center' : 'px-5 pb-6 pt-2'} md:p-8 md:text-left`}>
                <p className={`font-heading uppercase leading-[0.88] ${mobileFullscreen ? 'text-[2rem]' : 'text-[1.8rem]'} md:text-[1.8rem] ${textColor === 'white' ? 'text-white/80' : 'text-black/80'}`}>
                    <span className="block font-light">{label}</span>
                    <span className="block font-bold">{sublabel}</span>
                </p>
                <p className={`font-body-prototype ${mobileFullscreen ? 'mt-4 text-[1.0625rem] leading-[1.14]' : 'mt-3 text-[1rem] leading-[1.08]'} md:mt-4 md:text-[1rem] md:leading-[1.08] ${textColor === 'white' ? 'text-white/80' : 'text-black/80'}`}>
                    {description}
                </p>
            </div>
        </div>
        ...
        >
            ...
            >
                <div className="relative h-[7rem] w-[7rem] sm:h-[8rem] sm:w-[8rem] lg:h-[9.1875rem] lg:w-[9.1875rem]">
                    ...
                    />
                    <div
                        className={`absolute inset-[1.75rem] rounded-full bg-white transition-[transform,opacity] duration-300 ease-out group-hover:scale-0 group-hover:opacity-0 group-focus-visible:scale-0 group-focus-visible:opacity-0 sm:inset-[2rem] lg:inset-[2.5rem] ${mobilePopoverVisible ? 'scale-0 opacity-0' : ''}`}
                    />
                </div>
